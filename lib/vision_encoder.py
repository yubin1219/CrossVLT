...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.att_dr = attn_drop
        self.pj_dr = proj_drop
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, overlap=False, stride=1):
        ...
        if overlap:
            self.proj_overlap = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=stride)
        else:
...
class MMLayer(nn.Module):
    def __init__(self,
        ...
        if i_layer == 0:
            hdim = dim * 2
            # MHCA1 & 2
            self.attn1 = FusionLayer(dim, 768, dim=hdim, num_heads=int(hdim//64))
            self.attn2 = FusionLayer(hdim, 768, dim=hdim, num_heads=int(hdim//64)) 
            # FFN2
            self.linear2 = nn.Sequential(
            nn.Linear(hdim, hdim, bias=False),
            nn.ReLU(),
            nn.Linear(hdim,hdim, bias=False),
            nn.Tanh()
            )
            nn.init.zeros_(self.linear2[0].weight)
            nn.init.zeros_(self.linear2[2].weight)

        
        else:             
            # MHCA1
            self.attn1 = FusionLayer(dim, 768, dim=dim, num_heads=int(dim//64)) 
            if i_layer !=3:
                vdim = dim * 2
                hdim = dim * 2
                # MHCA2
                self.attn2 = FusionLayer(vdim, 768, dim=hdim, num_heads=int(hdim//64)) 
                # FFN2
                self.linear2 = nn.Sequential(
                    nn.Linear(vdim, vdim, bias=False),
                    nn.ReLU(),
                    nn.Linear(vdim, vdim, bias=False),
                    nn.Tanh()
                    )
        ...
        nn.init.zeros_(self.linear1[0].weight)
        nn.init.zeros_(self.linear1[2].weight)
    ...
    def forward(self, x, H, W, l, l_mask):
        ...
        x_proj = self.proj(x) # (B, HW, 256)
        
        # MHCA
        x_residual = self.attn1(x, l, l_mask)
        # FFN
        x = x + (self.linear1(x_residual) * x_residual)
        ...
        if self.downsample is not None:
            ...
            if self.i_layer==0:
                # MHCA
                x_res_down= self.attn2(x_down, l, l_mask)
                # FFN
                x_down = x_down + (self.linear2(x_res_down) * x_res_down)
                return  x_proj , x_residual, H, W, x_down, Wh, Ww
            # MHCA
            x_res_down= self.attn2(x_down, l, l_mask)
            # FFN
            x_down = x_down + (self.linear2(x_res_down) * x_res_down)
            ...
            return  x_proj , x_residual, H, W, x, H, W
...
class VisionStage(nn.Module):
    def __init__(self,
        ...
        super(VisionStage, self).__init__()
    ...
    def forward(self, x,  Wh, Ww, l, l_mask):
        """Forward function."""
        if self.i_layer==0:
        ...
        x_proj, x_res, H, W, x, Wh, Ww = self.layers(x, Wh, Ww, l, l_mask) 

        norm_layer = getattr(self, f'norm{self.i_layer}')
        x_out = norm_layer(x_res)  # output of a Block has shape (B, H*W, dim)

        out = x_out.view(-1, H, W, self.num_features[self.i_layer]).permute(0, 3, 1, 2).contiguous()
        
        if self.i_layer < 3:
            return x_proj, out, x, Wh, Ww, x

        return x_proj, out, x, Wh, Ww
        

class FusionLayer(nn.Module):
    def __init__(self, v_dim, l_dim, dim=196, num_heads=3):
